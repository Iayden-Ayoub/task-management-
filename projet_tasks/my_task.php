...
<head>
  ...
  align-items: center; /* باش يكون النص والأيقونة فالنص عموديًا */
  position: fixed;
  top: 0;
  left: 0;
  width: 100%;
  height: 50px;
  background-color: #333; /* خلفية غامقة */
  color: white; /* اللون ديال النص */
  padding: 0 20px; /* مسافة من اليمين واليسار */
  box-shadow: 0 2px 5px rgba(0, 0, 0, 0.2); /* ظل بسيط تحت البار */
   z-index: 1000;
}
...
<div id="idd" style="display:none; display:none; position: absolute; top: 60px; right: 20px; background:white; color:black; padding:10px; width: 300px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.2); z-index: 1001;">
  <?php
    require_once 'My_basse.php';
    $us = $_SESSION['user']['nom'];
  ...
  <div class="container" data-aos="fade-up">
    ...
    <div class="sidebar">
      ...
      <p><i class="fas fa-tachometer-alt"></i><a  href="home_user.php">Dashboard</a></p>
      <p class="active"><i class="fas fa-users-cog"></i><a href="my_task.php">My Task</a></p>
    ...
    <div class="content">
        ...
        <?php
            ...
            require_once 'My_basse.php';
            $po = $conn->prepare("SELECT * FROM task WHERE assigned = ? ");
            $po->execute([$_SESSION['user']['nom']]);
            $res = $po->fetchAll(PDO::FETCH_ASSOC);
?>
