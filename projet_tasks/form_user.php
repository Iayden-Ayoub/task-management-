...
<body>
    <div>
        ...
        <h1 style="margin-top: -100px;padding:30px;color:white">Bienvenue Empolye</h1>
...
if(isset($_POST['btn'])){
    ...
    $po = $conn->prepare('SELECT * FROM employe WHERE email = ? AND password = ?');
    $po->execute([$email,$password]);
    $res = $po->rowCount();
    if($res == 1){
        session_start();
        $_SESSION['user'] = $po->fetch(PDO::FETCH_ASSOC);
