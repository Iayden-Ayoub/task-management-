...
<div id="idd" style="display:none; position: absolute; top: 60px; right: 20px; background:white; color:black; padding:10px; width: 300px; border-radius: 8px; box-shadow: 0 0 10px rgba(0,0,0,0.2); z-index: 1001;">
  <?php
    require_once 'My_basse.php';
    $us = $_SESSION['user']['nom'];
    $po = $conn->prepare('SELECT message FROM notification WHERE nom = ? ORDER BY id DESC');
    $po->execute([$us]);
    $res = $po->fetchAll(PDO::FETCH_ASSOC);
    if(count($res) > 0){
      foreach($res as $r){
        echo "<p style='border-bottom:1px solid #ccc;padding:5px 0;'>".$r['message']."</p>";
      }
    }else{
  ...
  <div class="container" data-aos="fade-up">
    ...
    <div class="sidebar">
      ...
      <p class="active"><i class="fas fa-tasks"></i><a href="notifications.php">Notifications</a></p>
    ...
    <div class="content">
      ...
      <?php
            $us = $_SESSION['user']['nom'];
            $po = $conn->prepare('SELECT * FROM notification WHERE nom = ?');
            $po->execute([$us]);
            $res = $po->fetchAll(PDO::FETCH_ASSOC);
      ?>
      <table border="1">
